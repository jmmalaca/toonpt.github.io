...
var projectsLinks = [
  ...
  "https://www.citadelsecurities.com/valuation-metrics/"
]
...
function firstCommands(terminal, command) {
    if (command.match("^whoami$")) {
        ...
        terminal.echo("[" + green_text + "ϟ] [" + green_text + "ValuationMetrics], href=\"" + projectsLinks[6] + "\" @ MarsBased");
        terminal.echo("[" + green_text + "ϟ] [" + green_text + "Infopraca], href=\"" + projectsLinks[5] + "\" @ MarsBased");
...
function openWhat(terminal, command) {
    ...
    else if (what.match("[[Ii]nfopraca")) {
        link = projectsLinks[5];
    }
    else if (what.match("[[Vv]aluation[Mm]etrics")) {
        link = projectsLinks[6];
    }
    else {
        terminal.error(">Unknown command or some syntax error \"" + command + "\"" +
            "\n type \"help open\"... ;)");
